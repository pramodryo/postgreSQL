...
select * from movie;
select * from movie where title isnull;
...
GROUP BY name, total_votes,avg_rating
HAVING COUNT(m.id) >= 5
LIMIT 1;
...
FROM director_mapping as d
 ...
 date_difference AS
 (
 	 SELECT extract(day from next_movie_date - date_published) AS diff
...
select extract(day from now()-'2022-01-01')
 ...
 SELECT now(),Date_published, extract (day from now() - date_published) AS diff
	 FROM movie
	 
 SELECT extract (day from now() - date_published) AS diff
	 FROM movie