...
-- find all column names of movie Table.
...
select extract(month from date_published) as month, 
       count(title) as total_movies 
from movie 
group by month 
order by total_movies desc;
...
SELECT country, COUNT(id) AS number_of_movies, year
FROM movie
...
select country, 
      ...
      year 
from movie 
where year=2019 and country like any ('{"%India%","%USA%"}') 
group by country, year;
...
select g.genre, count(genre) as total_movie from genre g
left join movie m on 
m.id = g.movie_id group by g.genre order by total_movie desc limit 1;

-- OR
select genre, count(movie_id) as total_movie from genre group by genre order by total_movie desc;
...
with ct_genre as (
select movie_id, count(genre) as total_genre from genre group by movie_id having(count(genre) = 1)
)
select count(movie_id) as Number_of_movie from ct_genre
...
select g.genre, round(avg(duration),2) as average_duration from genre g
inner join movie as m
on m.id = g.movie_id group by g.genre order by average_duration desc;
...
select genre,
	  ...
	  rank() over(order by count(movie_id)desc) as genre_rank from genre group by genre
)
...
inner join movie as m
on m.id = g.movie_id
where m.country = 'USA' and r.total_votes >1000 and extract (month from date_published)= 3 and year=2017
group by g.genre
order by movie_count desc;
...
select  languages, sum(r.total_votes)
from movie as m
inner join ratings as r
on m.id = r.movie_id
...
SELECT DISTINCT name AS actor_name, round(avg(median_rating),2) as avg_rating, COUNT(r.movie_id) AS movie_count
FROM ratings AS r
...
FROM director_mapping as d
 ...
 date_difference AS
 (
 	 SELECT name_id, (next_movie_date - date_published) AS diff
 ...
 (
	 ...
	 GROUP BY director_id, name,avg_inter_movie_days,avg_rating
 )
 SELECT *	
 FROM final_result
 LIMIT 9;
...
select extract(day from now()-'2022-01-01')
