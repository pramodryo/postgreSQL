...
-- use the DMART scema;
set search_path to DMART, public;
 ...
 -- create dimensional table branch_details
...
-- Insert the records in customer table (4 types to inset values in table)
...
-- ## Analysis
-- DMART want to know analyse the customer behaviour
...
-- 3. How many orders where placed in month of january
select extract(month from order_date) as month, to_char(order_date, 'month') as month_name from sales_fact where extract(month from order_date)=1;

-- select all the cols
select * from sales_fact where extract(month from order_date)=1;

-- fetch the oredrs placed by customers
...
-- Right join Demo
-- selecting all the records from the right table irrespective of the matching key in the left table
select c.customer_id, c.first_name, c.last_name, c.gender, s.order_id, s.order_date from customer_info as c
...
-- *: used to selet all columns
-- from : keyword
-- table_name
-- where clause: apply filtering on the row/records


--sorting
select*from sales_fact order by customer_id asc;

select*from sales_fact order by customer_id desc;
...
-- count the number of cosumers that DMART own
select count(customer_id) as total_customer from customer_info;

-- Demonstrate the total value of the product: total list price
...
-- grouping: grouping by clause
-- find the male and female customers
select gender, count(customer_id) as total_count from customer_info group by gender;
...
-- find the premium customers who has placed more orders
 ...
 select customer_id, count(order_id) total_orders from sales_fact group by customer_id order by total_orders desc offset 1 limit 1;
 
 -- find the second highest and third highest customer on basis of order
 select customer_id, count(order_id) total_orders from sales_fact group by customer_id order by total_orders desc offset 1 limit 2;
 
 -- find the third highest customer on basis of order
 select customer_id, count(order_id) total_orders from sales_fact group by customer_id order by total_orders desc offset 2 limit 1;
...
-- refining above query to add the margin ratio
 ...
 group by customer_id having count(order_id) > 5 order by total_orders desc;
